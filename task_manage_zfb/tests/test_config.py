...
class TestConfigParserWithComments(unittest.TestCase):
    ...
    def test_ConfigParser_add_start_comment_raises_error(self):
        self.assertRaises(TypeError, self.config.add_start_comment, 1)
...
class TestConfig(unittest.TestCase):
    ...
    def test_Config__is_Config_File(self):
        self.my_config.write_configfile()
        self.assertFalse(Config._is_Config_File(self.normalfile_path))
        self.assertTrue(Config._is_Config_File(self.test_configfile_path))
    ...
    def tearDown(self):
        if os.path.isfile(self.test_configfile_path):
            os.remove(self.test_configfile_path)
        if os.path.isfile(self.normalfile_path):
            os.remove(self.normalfile_path)
        if os.path.isfile(self.emptyfile_path):
            os.remove(self.emptyfile_path)
